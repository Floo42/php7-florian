<?php
require_once (__DIR__.'/src/functions/getSerieArray.php');
require_once (__DIR__.'/src/functions/display_Stars.php');
require_once (__DIR__.'/src/functions/display_Genres.php');
...
<body>
    ...
    <main role="main">
        ...
        <div class="jumbotron" style="position: relative">
            <div class="jumbotron-background" style="background-image: url(<?= getSerieInfos($slug)['images']['banner']?>);"></div>
            <div class="container">
                <h1 class="display-3"><?= getSerieInfos($slug)['name']?></h1>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-3 d-none d-md-block">
                    <img src=<?= getSerieInfos($slug)['images']['poster']?> alt=<?= "Poster de " . getSerieInfos($slug)['name']?> class="img-thumbnail">
                </div>
                ...
                <div class="col-md-9">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">
                                <?= getSerieInfos($slug)['name']?>

                                <!-- Affichage de la note avec le bon nombre d'étoiles et un tooltip -->
                                <span class="stars text-info" data-toggle="tooltip" data-placement="top" title=<?= number_format(getSerieInfos($slug)['statistics']['rating'],2,"."," ")?>>
                                    <?= displayStars($slug) ?>
                                </span>
                            </h4>
                            <h6 class="card-subtitle mb-2 text-muted"><?= getSerieInfos($slug)['statistics']['season_count']?> saisons, <?= getSerieInfos($slug)['statistics']['episode_count']?> épisodes</h6>
                            <h5>
                                <?= number_format(getSerieInfos($slug)['statistics']['popularity'],0,"."," ")?> personnes suivent la série

                            </h5>
                            <p>
                                <!-- Affichage des genres de la série -->
                                <?= displayGenres($slug) ?>
                                <small>sortie en <?= getSerieInfos($slug)['release_year']?> chez <?= getSerieInfos($slug)['network']?></small>
                            </p>
                            <p class="card-text">
                                <?= getSerieInfos($slug)['synopsis']?>
                            </p>
                            <a target="_blank" href=<?= "https://www.betaseries.com/serie/" . getSerieInfos($slug)['slug']?> class="card-link">
                    ...
                    <div class="card">
                        ...
                        <div class="embed-responsive embed-responsive-21by9">
                            ...
                            <iframe width="560" height="315" src=<?= "https://www.youtube.com/embed/" . getSerieInfos($slug)['youtube_id'] . "?rel=0&amp;showinfo=0" ?> frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
                        </div>
                    </div>
                </div>
            </div>
